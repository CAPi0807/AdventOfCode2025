...
He aplicado una arquitectura basada en el **Principio de Inversión de Dependencias (DIP)**. El servicio principal no depende de una lógica rígida, sino de una abstracción (`NumberPredicate`), lo que nos permite cambiar las reglas de filtrado de números sin tocar una sola línea del motor de cálculo.
...
3. **`distinct`**: Elimina duplicados si los rangos se solapan.
...
- **OCP**: ¿Mañana queremos otra lógica? Solo creamos un `XNumberPredicate` y lo pasamos al constructor del servicio. El código existente no se toca.
...
Lo más destacable de esta solución es que **no he modificado ni una sola línea** del servicio de suma o del parser de la parte A. Simplemente he cambiado la "estrategia" de filtrado.



## 2. El Nuevo Corazón: `PatternRepetitivePredicate`
Este archivo contiene una lógica un poco chunga. A diferencia de la otra versión, esta utiliza un enfoque de **búsqueda de patrones dinámicos**:
...
## 3. Reutilización de Componentes de la Parte A
En el `Main`, importamos clases del paquete `day02.a`:
...
- **DIP (Inversión de Dependencias)**: El servicio depende de la interfaz `NumberPredicate` y no de una clase concreta.
- **DRY (Don't Repeat Yourself)**: Imprtamos código de la parte A.
- **Escalabilidad**: La interfaz es implementable de la menera que se quiera.
